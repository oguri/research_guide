...
論文のlatexファイルや図のファイル、また計算や解析のコードや重要なデータなどは必ずこまめにバックアップをとるようにしてください。自分のPCがいきなり壊れる、また間違えてディレクトリごと全部消去してしまう、ということはしばしば発生しますが、そのような時に速やかに復旧できる体制を整えておくことが重要です。自分のPC内の違うディレクトリにコピーするだけだと、PCが壊れた場合どうしようもないので、外部ディスクやクラウドも活用して二重、三重にバックアップをとるのが良いです。rsync+ssh(鍵認証)+cronないしlaunchctl、あるいはMacの場合はTime Machineを使うことで、自動で定期的にバックアップをとる仕組みを構築することができます。Google driveなどを活用するのも良いと思います。
...
* 時間に余裕をもって依頼するようにしましょう。どのくらい余裕を持つかはケースバイケースなので一概には言えませんが、例えば共同研究者に論文を回覧するときは2, 3週間くらいの期間をとることが一般的です。学位論文など締め切りがある場合に、締め切り直前に依頼しても、チェックする側が時間の確保が難しい場合もあり、迷惑をかけることになるので気をつけてください。時間に余裕がないと、十分な修正案やコメントをもらえず、自分自身の不利益にも繋がります。私が修士論文をチェックする際の標準的なスケジュールは、締め切りの1か月前に一通り書き上げてもらって、それから締め切りまで構成や内容、体裁をブラッシュアップしていくというもので、多くの場合これでもぎりぎりのスケジュールになります。
