...
論文に掲載する図を作るときは、装置やシミュレーションの画像ではなく線や点やラベルを書く普通のプロット (いわゆるグラフ) に対しては、ベクター画像のPDFで作成する方が見栄えが良いです。ベクター画像にすると、拡大しても解像度が落ちないなどの利点があります。一方でPNG形式やJPEG形式はビットマップ画像なので、拡大すると解像度が落ちるので論文に載せるグラフの図として使うのはおすすめしません。ベクター画像のPDFは、例えばmatplotlibではsavefigで.pdfの拡張子で保存することで作成できます。
